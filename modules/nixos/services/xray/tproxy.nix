{ config, pkgs, ... }:
let
    port = toString config.my.services.xray.tproxyPort;
in
{
    config = {
        ...
        networking.nftables.ruleset = ''
            table inet xray {
                chain prerouting {
                    ...
                    ip daddr { 100.64.0.0/10, 127.0.0.0/8, 169.254.0.0/16, 172.16.0.0/12, 224.0.0.0/4, 255.255.255.255 } return
                    ...
                    meta l4proto { tcp, udp } meta mark set 0x00000001 tproxy ip to 127.0.0.1:${port} accept
                    meta l4proto { tcp, udp } meta mark set 0x00000001 tproxy ip6 to [::1]:${port} accept
                }
